...
export type CategoryValidationResult =
  { success: true; data: CategoryInput } | { success: false; error: string };

export function validateCategoryInput(body: unknown): CategoryValidationResult {
  if (typeof body !== "object" || body === null) {
    return { success: false, error: "Invalid request body" };
  }
  const { name, parentCategoryId, description } = body as Record<
    string,
    unknown
  >;
  ...
  if (parentCategoryIdResult === undefined) {
    return {
      success: false,
      error: "parentCategoryId must be a number or null",
    };
  }
