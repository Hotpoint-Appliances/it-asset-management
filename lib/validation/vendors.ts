...
export function validateVendorInput(body: unknown): VendorValidationResult {
  ...
  const { name, contactName, contactEmail, contactPhone, address } = body as Record<
    string,
    unknown
  >;
  ...
  if (contactEmailResult === undefined || (contactEmailResult && !contactEmailResult.includes("@"))) {
    return { success: false, error: "contactEmail must be a valid email or null" };
  }
